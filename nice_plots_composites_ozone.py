...
for ii, name_vortex in enumerate(name_vortices_list):
 ...
 field_PHR = get_anomalie(aa, var=var, doincr=False, doanom=True)
 field = get_anomalie(aa, var='O3', doincr=True, doanom=False)
 ...
 xx_b, yy_b = slanted_axis(field.xx.values, alpha)
 ...
 cbar.set_label('('+units+')'+'/day')


 if (First):
     xrepere = 1./ffac*coupe_horizontale.xx.values[15]
 ...
 if (First):
     First= False
     xtitle = 'O$_3$ anomaly'
     ytitle = 'O$_3$ increment tendency'
...
plt.savefig('./figures_papier/nice_composites_ozone.pdf')#,dpi=300)
plt.show()
