...
for ii, name_vortex in enumerate(name_vortices_list):
 ...
 field_vor =  get_anomalie(aa, var='VO')
 ...
 if (First):
     xrepere = 1./ffac*coupe_horizontale.xx.values[18]
 ...
 ax4 = plt.subplot(nbase+2)


 coupe_horizontale_vor = field_vor.interp(xx=('xhor', xx_b), yy=('xhor', yy_b)).rename(var)
 coupe_hor_vo = VO.interp(xx=('xhor', xx_b), yy=('xhor', yy_b)).rename('VO')
 ...
 '''
 lev_cols=20
 cf4= ax4.contourf(coupe_horizontale_vor.xx.values, coupe_horizontale_vor.zz.values, coupe_horizontale_vor.values, levels = lev_cols, cmap = cmap, extend='both')
 ax4.contour(coupe_horizontale.xx.values, coupe_horizontale.zz.values,coupe_hor_vo.values,colors='g',levels = seuils)
 xlab = 'x$_b$ (km)' if (np.abs(alpha)>1.e-2) else 'x (km)'
 ax4.set_xlabel(xlab)
 ax4.set_ylabel('z (km)')
 ...
 if ((var == 'T') or var=='PV'):
     ...
     CS = ax4.contour(coupe_horizontale.xx.values, coupe_horizontale.zz.values, coupe_horizontale.values, levels = levs,colors='k')
     ax4.clabel(CS, fontsize=10, fmt = '%1.1f')#, inline=1)

 #pos = np.array(matplotlib.artist.getp(ax2, 'position'))
 #cbar_ax = fig.add_axes([0.92, pos[0,1], 0.02, pos[1,1]-pos[0,1]])
 cbar = plt.colorbar(cf4, orientation = 'vertical')
 cbar.formatter.set_powerlimits((0, 0))
 cbar.update_ticks()
 cbar.set_label('s$^{-1}\,$d$^{-1}$')
 ...
 if (First):
     ...
     ax4.set_title('$\\zeta$ increment tendency', y=ypos)
 for ij, ax in enumerate([ax2, ax3, ax4]): 
     ik = 3*ii + ij
     ax.text(-0.1, 1.03, ind_plot[ik], transform=ax.transAxes, size=14, weight='bold')

plt.savefig('./figures_papier/nice_composites_heating_rate_vo.pdf')#,dpi=300)
plt.show()
